...
<body style="background-color: #f3f3f3;">
    ...
    <div class="container mt-4">
        <h1 class="text-center">My dating history</h1>

        <form id="horarios-form" method="post" action="verificar_contraseña.php">
    <div class="form-group">
        <label for="usuario">Select your user:</label>
        <select name="usuario" id="usuario" class="form-control">
            <?php
            //opciones del dropdown con los nombres de los usuarios
    ...
    <div class="form-group">
     ...
     <button type="submit" class="btn btn-primary">See My Schedules</button>
      </form>
    ...
    $('#horarios-form').submit(function (event) {
        ...
        $.ajax({
            type: 'POST',
            url: 'verificar_contraseña.php',
            data: { usuario: selectedUser, password: inputPassword },
            success: function (response) {
                if (response === 'success') {
                    // Obtener nuevamente la lista de usuarios después de la verificación exitosa
                    $.ajax({
                        type: 'GET',
                        url: 'obtener_usuarios.php', // Reemplaza con la ruta correcta
                        success: function (usuarios) {
                            // Actualizar el dropdown con los nuevos nombres de usuario
                            $('#usuario').empty();
                            $.each(usuarios, function (index, usuario) {
                                $('#usuario').append('<option value="' + usuario.ID + '">' + usuario.name + '</option>');
                            });
                        },
                        error: function () {
                    ...
                    // Redirigir al usuario a la página de horarios
                    window.location.href = 'obtener_horarios.php';
                } else {
